...
from src.pipeline.parse import _add_hierarchy, parse_document
...
class TestParsing:
    ...
    def test_multiline_section_headers(self, parsed_sections):
        ...
        if "5.4.8.2" in parsed_sections:
            ...
            assert expected in title, f"Multi-line header not properly extracted. Got: {title}"
        ...
        if "5.5.1" in parsed_sections:
            ...
            assert title == expected_full, f"Section 5.5.1 title should be complete. Got: {title}"
...
class TestRuleExtraction:
    ...
    def test_extract_rules_has_required_fields(self, mock_openai_client, sample_section_data):
