...
from src.pipeline.validate import validate_and_filter_questions, generate_validation_analysis
    ...
    for section_id, section_rules in rules_by_section.items():
        ...
        for rule_index, rule in enumerate(section_rules):
            ...
            questions = generate_questions_for_rule(
                rule, section_id, rule_index, client,
...
def cmd_validate(args):
    ...
    print_summary(
        "VALIDATION RESULTS",
        {
            f"Total questions": validation_report['total_questions'],
            f"Validated": validation_report['validated'],
            f"Rejected": validation_report['rejected'],
            f"Structural failures": validation_report['structural_failures'],
            f"Quality failures": validation_report['quality_failures'],
        }
    )
...
def cmd_all(args):
    ...
    if args.resume and rules_output.exists():
        ...
        print("=" * 60)
        rules_args = type('obj', (object,), {
            'input': str(parse_output),
            'section': args.section,
    ...
    if args.resume and questions_output.exists():
        ...
        print("=" * 60)
        questions_args = type('obj', (object,), {
